...
async function checkExistingRSVP() {
	...
	try {
		...
		if (!user) {
			submitError.value =
				"We can't seem to find you. Please double check the name inputed is correct or contact us for help."
			resetForm()
			return
		}
		...
		if (user.isAttending === true) {
			...
			if (user.pickUpDropOff !== undefined) {
				showBusOptions.value =
					form.value.pickUpDropOff !== BusLocation.NotRequired
			}
		}
	} catch (error) {
		submitError.value =
			'An error occurred while checking your RSVP. Please try again.'
...
async function handleSubmit() {
	try {
		if (!exisitingUserId.value) {
			submitError.value =
				'No existing RSVP found. Please check your name first.'
			return
		}
		...
		if (form.value.isAttending === true) {
			if (!form.value.pickUpDropOff) {
				if (!showBusOptions.value) {
					submitError.value =
						'Please let us know if you would like to use the bus.'
					return
				}

				submitError.value =
					'Please choose a pick-up/drop-off location for the bus.'
				return
			}
			...
			if (!form.value.allergyOrIntolerance) {
				if (!showAllergyInput.value) {
					submitError.value =
						'Please let us know if you have any allergies/intolerances.'
					return
				}
...
async function createBaseUser() {
	...
	await setDoc(doc(db, 'submissions', submissionId), {
		firstName: form.value.firstName.trim().toLowerCase(),
...
<template>
	<div id="rsvp-form" class="rsvp">
		<div class="rsvp-container" v-if="!submitSuccess">
			<form
				@submit.prevent="checkExistingRSVP"
				class="rsvp-form"
				v-if="!exisitingUserId"
			>
			...
			<form @submit.prevent="handleSubmit" class="rsvp-form" v-else>
				...
				<h2 class="rsvp-subtitle rsvp-subtitle-right">
				...
				<div class="form-group" v-if="form.isAttending !== false">
					...
					<label class="form-label"
						>Will you require bus transport to and from the venue? *</label
					>
					<div class="radio-group">
						<div class="radio-option">
							<input
					...
					<div class="form-group" v-if="showBusOptions">
						<label class="form-label"
							>Which pick-up/drop-off location would you prefer?</label
						>
						<div class="radio-group">
							<div class="radio-option">
								<input
					...
					<label class="form-label"
						>Do you have any Allergies/Intolerances?</label
					>
					<div class="radio-group">
						<div class="radio-option">
							<input
					...
					<div class="form-group" v-if="showAllergyInput">
						<label for="allergies-list"
							>Please list your Allergies/Intolerances</label
						>
						<input
							id="allergies-list"
					...
					<div class="form-group" v-if="showDietPreferenceInput">
						<label for="dietary-list"
							>Please list your Dietary Preferences</label
						>
						<input
							id="dietary-list"
					...
					<label class="form-label"
						>Would you like the chance to be the Ring Bearer? (Birds
						involved)</label
					>
					<div class="radio-group">
						<div class="radio-option">
						...
						<div class="radio-option">
							...
							/>
							<span class="custom-radio-button"></span>
							<label for="ring-bearer-no"
								>That's eggciting, but I'm too hawkward for the job</label
							>
						</div>
					</div>
				</div>
		...
		<div v-else class="rsvp-container">
			<div class="rsvp-form" v-if="isGoing === true">
				...
				<h4 class="rsvp-form-message">
					We have received your RSVP and will be in touch soon!
				</h4>
			</div>
			<div class="rsvp-form" v-else>
				...
				<h4 class="rsvp-form-message">
					We have received your RSVP, but we understand you won't be able to
					join us.
				</h4>
			</div>
		</div>
